...
pad_beat = Beat(time=pad_noteTime, taps=4, w1=100, w2=100, w3=100).play(delay=pad_noteTime*2)
...
bass_beat = Beat(time=bass_noteTime, taps=4, w1=100, w2=100, w3=100).play(delay=bass_noteTime*4)
...
## Melo
melo_noteTime = 0.125
melo_notes = [midiToHz(x) for x  in [48 ,53 ,64,48 ,53 ,62,48 ,55 ,62,48 ,55 ,60]]
melo_env = ExpTable([ ( 0 , 0 ) , (400 ,1) , (500 ,1) , (1000 ,.5) , (5000 ,0) ] , size =8192)
melo_beat = Beat(time=melo_noteTime, taps=16, w1=80, w2=50, w3=30, poly=1, onlyonce=False).play()
melo_trigChoice = TrigChoice(melo_beat, choice=melo_notes, port=0.005)
melo_amp = TrigEnv(melo_beat, table=melo_env, dur=melo_noteTime, mul=1)



melo_instru = SineLoop(freq=melo_trigChoice, feedback=0, mul=melo_amp, add=0).mix(2).out()
